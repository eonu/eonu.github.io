...
\begin{align}
	y^{(i)}
	&=\begin{cases}
    1\quad & \text{if} \quad \cp{y^{(i)}=1}{\b{x}^{(i)}}\geq\frac{1}{2}\\
    0      & \text{otherwise}
	\end{cases}\\
	&\ \text{Or equivalently:}\\
	&=\begin{cases}
		0\quad & \text{if} \quad \cp{y^{(i)}=0}{\b{x}^{(i)}}>\frac{1}{2}\\
		1      & \text{otherwise}
	\end{cases}
\end{align}
$$

> **Note**: Observe that $\cp{y^{(i)}=1}{\b{x}^{(i)}}$ and $\cp{y^{(i)}=0}{\b{x}^{(i)}}$ must form a probability distribution, meaning that:
> $$
> \cp{y^{(i)}=1}{\b{x}^{(i)}}+\cp{y^{(i)}=0}{\b{x}^{(i)}}=1
> $$

Where the conditional probability $\cp{y^{(i)}=1}{\b{x}^{(i)}}$ is the result of applying the logistic function to $\bs{\theta}^\T\b{x}^{(i)}$ as seen before:
$$
\begin{align}
	\cp{y^{(i)}=1}{\b{x}^{(i)}}
	&=\sigma\l(\bs{\theta}^\T\b{x}^{(i)}\r)\\
...
> **Note**: These conditional probabilities are parameterized by $\bs{\theta}$.
...
In the case of binomial logistic regression, this decision boundary is linear—a **hyperplane**. The decision boundary would occur when $\cp{y^{(i)}=1}{\b{x}^{(i)}}=\frac{1}{2}$, as we are completely uncertain which class to assign when the probability is $\frac{1}{2}$:
$$
\begin{align}
	\cp{y^{(i)}=1}{\b{x}^{(i)}}&=\frac{1}{2}\\
...
> **Note**: Observe that the decision boundary is linear in $\b{x}^{(i)}$.
...
As we have a binary output variable with probability $p=\cp{y^{(i)}=1}{\b{x}^{(i)}}$ of taking value $1$, and probability $q=\cp{y^{(i)}=0}{\b{x}^{(i)}}$ (or $1-p=1-\cp{y^{(i)}=1}{\b{x}^{(i)}}$) of taking value $0$, we can say that the output variable is distributed as a Bernoulli random variable.
$$
y^{(i)} \sim \mathrm{Bernoulli}\l(p^{(i)}\r)
$$

> **Where**: $p^{(i)}=\cp{y^{(i)}=1}{\b{x}^{(i)}}$ is the parameter for the distribution.
...
The ==**likelihood function**== $L$ for the Bernoulli distributed output variable $y^{(i)} \sim \mathrm{Bernoulli} \l(p^{(i)}\r)$ is given by:
$$
L\l(p^{(i)}; \bs{\theta}\r)=\prod_{j=1}^n \l(p^{(i)}\r)^{y^{(i)}} \l(1-p^{(i)}\r)^{1-y^{(i)}}
$$
...
$$
\forall (x_1,x_2)\in\R^2\land x_1,x_2>0,\quad x \geq y \implies \log x_1\geq \log x_2
$$
We can now write the log-likelihood for the output variable as:
$$
\begin{align}
	\mathcal{L}\l(p^{(i)}; \bs{\theta}\r)
	&=\log L\l(p^{(i)}; \bs{\theta}\r)\\
...
$$
\hat{\bs{\theta}}=\arg\max_{\bs{\theta}\in\bs{\Theta}} \mathcal{L}\l(p^{(i)}; \bs{\theta}\r)
$$
...
\begin{align}
	\hat{\bs{\theta}}
	&=\arg\min_{\bs{\theta}\in\bs{\Theta}} -\mathcal{L}\l(p^{(i)}; \bs{\theta}\r)\\
	&=\arg\min_{\bs{\theta}\in\bs{\Theta}} \mathcal{L}^-\l(p^{(i)}; \bs{\theta}\r)
\end{align}
$$

> **Where**: $\mathcal{L}^-$ denotes the **==negative log-likelihood==** $-\mathcal{L}$, which is the **error function** $C(\bs{\theta})$ for our model.

> In the *Linear Regression* notes, we saw that the following minimization problem for OLS:
> $$
> \hat{\bs{\theta}}=\arg\min_{\bs{\theta}\in\bs{\Theta}}\l(\b{y}-\bs{\Phi}\bs{\theta}\r)^\T\l(\b{y}-\bs{\Phi}\bs{\theta}\r)
> $$
> Had a closed-form (analytical) solution—$\hat{\bs{\theta}}=\l(\bs{\Phi}^\T\bs{\Phi}\r)^{-1}\bs{\Phi}^\T\b{y}$.

This is not the case for the MLE in logistic regression. We must take the derivative of $\mathcal{L}^-$ with respect to $\bs{\theta}$. This can be done by taking partial derivatives, since:
$$
\nabla\mathcal{L^-}
	=\deriv{\mathcal{L}^-}{\bs{\theta}}=\l(
		\pderiv{\mathcal{L}^-}{\theta_0},
		\pderiv{\mathcal{L}^-}{\theta_1},
		\ldots,
		\pderiv{\mathcal{L}^-}{\theta_n}
	\r)
$$
> **Where**: $\nabla\mathcal{L}^-$ is called the **==gradient==** (in vector calculus).

An individual partial derivative with respect to $\theta_j$ would be given by:
$$
\begin{align}
	&\ \text{Simplifying:}\\
	
	\pderiv{\mathcal{L}^-}{\theta_j}
...
The gradient $\nabla\mathcal{L}^-$ points in the direction of steepest **ascent**. We are interested in "taking a step" in the direction of steepest descent, $-\nabla\mathcal{L}^-$.

By "taking a step", we mean moving some distance $\alpha$ in a particular direction in $\bs{\theta}$-space.
...
A step of size $\eta$ in the direction of steepest descent $-\nabla\mathcal{L}^-$ (in $\bs{\theta}$-space) would be represented by the vector $-\eta\nabla\mathcal{L}^-$. If we take our current position $\bs{\theta}$ and apply this step by adding the transformation vector $-\eta\nabla\mathcal{L}^-$, we arrive at our updated position:
$$
...
This would be repeated until convergence. It is also possible to update individual components separately for this step—updating one component **once** then moving on to the next:
$$
\theta_j^\bluet{new} \leftarrow \theta_j^\bluet{old}-\eta\pderiv{\mathcal{L}^-}{\theta_j}
$$

##### Coordinate descent

It is also possible to consider an individual direction or component $\theta_j$ at a time when taking steps—completely focusing one component, but updating it **multiple** times to minimize this component before returning to minimize the others.
$$
\theta_j^\bluet{new} \leftarrow \theta_j^\bluet{old}-\eta\pderiv{\mathcal{L}^-}{\theta_j}
$$
...
- _Wikipedia_<br/>[Decision boundary](https://en.wikipedia.org/wiki/Decision_boundary)<br/>[Logistic regression](https://en.wikipedia.org/wiki/Logistic_regression)<br/>[Monotonic function](https://en.wikipedia.org/wiki/Monotonic_function)<br/>[Sigmoid function](https://en.wikipedia.org/wiki/Sigmoid_function)<br/>[Bernoulli distribution](https://en.wikipedia.org/wiki/Bernoulli_distribution)
